...
| 17. Documentation polish | Complete | README is now a concise repo entrypoint; detailed setup, config, adapter, CI, incident, and sample docs live in `docs/USER_GUIDE.md`. |
| 18. Quality gate | Complete | `npm test`, `typecheck`, `build`, `audit`, and relevant CLI profile/generate smoke pass for current slice. |
| 19. Suite and run versioning | Complete | Latest-only artifacts are the default; opt-in suite/run history, IDs, hashes, config toggle, CLI/API wiring, docs, and tests are in place. |
| 20. Local project studio | Planned | Add a Swagger-like `quainy-risk-replay studio` UI that runs from the user's project root and uses the same local engine as the CLI. |
...
- [x] Reports include suite/run trace metadata so each run identifies the generated suite it used.
...
- [x] Writes reports.
- [x] Prints suite and run identifiers.
...
- [x] config reference
- [x] adapter guide
- [x] exact adapter request/response JSON contract
- [x] GitHub Actions guide
- [x] incident workflow guide
- [x] suite generation explanation
- [x] report interpretation guide with sample Markdown, JSON shape, field meanings, and repeated-run workflow
- [x] curl/PowerShell pre-npm install guide
- [x] detailed documentation split out of README into `docs/USER_GUIDE.md`
- [x] user guide ordered as a real workflow from config to generation to adapter/run to report interpretation
...
- [x] README, user guide, installers, and dashboard explain that the installed CLI is for external projects while the showcase app is repo-local.
...
## Phase 19: Suite And Run Versioning

Make generation and execution traceable while keeping latest-only overwrite behavior as the default.

Required behavior:

- [x] Generated suite writes `risk-replay/tests/generated-suite.json`.
- [x] Generated suite writes `risk-replay/tests/versions/suite-<timestamp>-<hash>.json` when versioning is enabled.
- [x] Suite artifact includes suite ID, suite hash, config hash, context hash, test count, and risk surface count.
- [x] Run report writes `risk-replay/reports/latest.json` and `risk-replay/reports/latest.md`.
- [x] Run report writes `risk-replay/reports/runs/run-<timestamp>-<hash>.*` when versioning is enabled.
- [x] Run report includes `suiteId`, `suiteHash`, `suitePath`, and `suiteVersionPath`.
- [x] Markdown report includes suite-level summary and per-test result table.
- [x] CLI output prints suite ID and run ID.
- [x] Dashboard state exposes suite and run identifiers.
- [x] Config supports `"versioning": { "enabled": true }` to keep suite and run history.
- [x] GitHub Actions artifact upload includes versioned suite and run artifacts.
- [x] README and user guide explain rerun behavior and the versioning toggle.

Tests:

- [x] Suite writer creates latest-only artifacts by default.
- [x] Suite writer creates versioned artifacts when enabled.
- [x] Run report references the exact suite used.
- [x] Markdown report exposes per-test result details.

Acceptance:

- [x] A user can identify which generated suite produced any saved run report.
- [x] A user can disable history files without changing generation or run commands.

## Phase 20: Local Project Studio

Turn the current showcase UI direction into a real user-facing local UI, similar in spirit to Swagger UI for FastAPI apps.

Purpose:

- [ ] Let users configure, generate, run, and inspect Risk Replay from a browser UI inside their own AI project.
- [ ] Avoid requiring users to copy this whole repository into their app.
- [ ] Keep the security model local-first: no repo upload, no Quainy server dependency, no broad file reads.

Required behavior:

- [ ] Add `quainy-risk-replay studio`.
- [ ] Command runs from the target AI project root.
- [ ] Command starts a local-only web server on `localhost`.
- [ ] UI reads the target project's `risk-replay.config.json`.
- [ ] UI displays discovered local context and profile provenance.
- [ ] UI lets users edit or confirm missing profile fields.
- [ ] UI generates the suite using the same core generator as CLI.
- [ ] UI shows generated tests with risk category, severity, why, pass criteria, fail criteria, and linked profile fields.
- [ ] UI lets users add incident regressions.
- [ ] UI runs the configured adapter using the same runner as CLI.
- [ ] UI displays suite-level report details.
- [ ] UI displays per-test result details.
- [ ] UI writes the same local artifacts as CLI.
- [ ] UI respects `versioning.enabled`.
- [ ] UI supports read-only/report mode for CI artifacts later.

Acceptance:

- [ ] A user can install the tool, run `quainy-risk-replay studio` from their AI project, and use the browser UI without cloning the Quainy Risk Replay repo into their app.
- [ ] The studio UI and CLI produce compatible suite and report artifacts.
